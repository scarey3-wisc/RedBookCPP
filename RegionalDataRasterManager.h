...
inline static constexpr int REGIONAL_DATA_DIMENSION = 256;
...
using RegionalDataRasterManager = DataRasterManager<DataType, REGIONAL_DATA_DIMENSION, RegionalDataLoc, RegionalDataLocHasher, Policy>;

template <typename DataType, typename Policy = DataRasterDefaultPolicy>
using RegionalDataHandle = RegionalDataRasterManager<DataType, Policy>::DataRasterHandle;
...
private:
	std::vector<std::unique_ptr<RegionalDataRasterManager<DataType, Policy>>> managers;
...
public:
	MultiLODRegionalDataRasterManager(const std::array<int, REGIONAL_MAP_NUM_LODS>& cacheSizes, WorldMap* myWorld, RegionalDataRequestStack* pool)
	{
		...
		{
			managers.emplace_back(std::make_unique<RegionalDataRasterManager<DataType, Policy>>(cacheSizes[i], myWorld, pool));
		}
	}
	RegionalDataHandle<DataType, Policy> GetRaster(const RegionalDataLoc& id)
	{
	...
	{
		RegionalDataHandle<DataType, Policy> raster = GetRaster(id);
